...
import { BackButton, GradientButton, SuccessDialog } from '@/app/components';
...
export default function Profile() {
  ...
  // Initialize profile data; avoid deriving from email. DB effect will populate fullName
  const getInitialProfileData = () => {
    return {
      fullName: '', // Start empty, will be populated from database
  ...
  useEffect(() => {
    ...
    const fetchFullName = async (retryCount = 0) => {
      try {
        console.log('Profile: Fetching fullName for UID:', uid, 'retry:', retryCount);
        const snap = await get(ref(rtdb, `users/${uid}/fullName`));
        if (snap.exists()) {
          const fullName = String(snap.val());
          console.log('Profile: Found fullName:', fullName);
          setProfileData((prev) => ({ ...prev, fullName }));
          setFormData((prev) => ({ ...prev, fullName }));
        } else {
          console.log('Profile: No fullName found in Firebase for UID:', uid);
          // Check if user data exists at all
          const userSnap = await get(ref(rtdb, `users/${uid}`));
          if (userSnap.exists()) {
            const userData = userSnap.val();
            console.log('Profile: User data exists:', userData);
            // Try to get fullName from the user data object
            if (userData.fullName) {
              console.log('Profile: Found fullName in user data:', userData.fullName);
              setProfileData((prev) => ({ ...prev, fullName: userData.fullName }));
              setFormData((prev) => ({ ...prev, fullName: userData.fullName }));
            } else if (retryCount < 3) {
              // Retry after a short delay if fullName not found
              console.log('Profile: Retrying fetch in 1 second...');
              setTimeout(() => fetchFullName(retryCount + 1), 1000);
            }
          } else {
            console.log('Profile: No user data found in Firebase for UID:', uid);
            if (retryCount < 3) {
              // Retry after a short delay if user data not found
              console.log('Profile: Retrying fetch in 1 second...');
              setTimeout(() => fetchFullName(retryCount + 1), 1000);
            }
          }
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        if (asset.fileSize) {
          ...
          console.log(`📸 Profile image captured: ${asset.uri}`);
          console.log(`📏 Image size after compression: ${sizeInKB} KB (${sizeInMB} MB)`);
          console.log(`⚡ Quality setting: 10% (instant upload)`);
        } else {
          console.log(`📸 Profile image captured: ${asset.uri} (compressed for instant upload)`);
        }
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        if (asset.fileSize) {
          ...
          console.log(`📸 Profile image selected: ${asset.uri} (compressed for instant upload)`);
        }
    ...
    <View style={styles.container}>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.inputField}>
            ...
            <View style={styles.inputContainer}>
              ...
              />
              <Text style={styles.inputText}>{formData.fullName || 'Enter your full name'}</Text>
            </View>
          </View>
      ...
      <View style={{
        ...
        <GradientButton
          ...
          onPress={async () => {
            // Ensure name exists (from DB); if not, inform user to set it via support
            if (!formData.fullName || formData.fullName.trim() === '') {
              alert('Full name not found in your profile. Please contact support to update.');
              return;
            }
