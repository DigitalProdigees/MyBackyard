...
export default function iOSPaymentProcessing() {
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      ...
      console.log('iOS Payment Processing: Initializing component');
      ...
      console.log('iOS Payment Processing: Reset all payment state flags on mount');

      const listingId = params.listingId as string;
      const total = params.total as string;
      
      if (!listingId || !total) {
        console.log('iOS Payment Processing: Missing required params on mount, redirecting to home');
        router.replace('/(main-app)/home');
        return;
      }

      console.log('iOS Payment Processing: Params validated, ready for payment');
    };
    ...
    return () => {
      ...
      hasStartedPaymentRef.current = false;
      setIsReturningFromStripe(false);
      console.log('iOS Payment Processing: Component cleanup completed');
    };
  }, []);

  // Handle payment cancellation - iOS specific
  const handlePaymentCancellation = useCallback(async () => {
    console.log('iOS Payment Processing: Handling payment cancellation');
    setPaymentState('cancelled');
    
    // Set cancellation flag to prevent automatic reopening
    hasBeenCancelledRef.current = true;
    
    // iOS-specific: Ensure WebBrowser is dismissed
    try {
      await WebBrowser.dismissBrowser();
      console.log('iOS WebBrowser dismissed after cancellation');
    } catch (error) {
    ...
    console.log('iOS Payment Processing: Payment cancelled, waiting for user action');
  }, []);

  // Create Stripe Checkout Session - iOS optimized
  const createCheckoutSessioniOS = useCallback(async () => {
    // Simple guard: only prevent if currently processing
    if (isProcessingRef.current) {
      console.log('iOS Payment Processing: Checkout already in progress, skipping');
      return;
    }
    ...
    if (currentSessionId) {
      ...
      if (!isSessionValid) {
        console.log('iOS Payment Processing: Existing session is no longer valid, creating new one');
        setCurrentSessionId(null);
      } else {
        console.log('iOS Payment Processing: Existing session is still valid, reopening checkout');
        // Reopen the existing checkout URL if we have it
        if (checkoutUrl) {
          const result = await WebBrowser.openBrowserAsync(checkoutUrl, {
            dismissButtonStyle: 'cancel',
            presentationStyle: WebBrowser.WebBrowserPresentationStyle.FORM_SHEET, // iOS prefers form sheet
          });
    ...
    try {
      console.log('iOS Payment Processing: Starting createCheckoutSession');
      ...
      if (sessionId) {
        ...
        console.log('iOS Payment Processing: Session ID received:', sessionId);
      } else {
      ...
      // iOS-specific: Open Stripe Checkout with form sheet presentation
      ...
      console.log('iOS WebBrowser result:', result);
      
      // Handle the result when user returns
      if (result.type === 'dismiss' || result.type === 'cancel') {
        // User cancelled or closed the browser
        console.log('iOS Payment cancelled by user');
        setPaymentState('cancelled');
        setIsProcessing(false);
        isProcessingRef.current = false;
        hasStartedPaymentRef.current = false;
        ...
        console.log('iOS Stripe checkout opened successfully');
        // Keep processing state until deep link or cancellation
      } else {
        // Handle any other result type as cancellation
        console.log('iOS WebBrowser returned unexpected result, treating as cancellation');
        setPaymentState('cancelled');
        setIsProcessing(false);
        isProcessingRef.current = false;
        hasStartedPaymentRef.current = false;
      ...
      console.log('iOS Checkout error:', error);
  ...
  useEffect(() => {
    const startPaymentProcess = async () => {
      ...
      console.log('iOS Payment Processing: Checking params - listingId:', listingId, 'total:', total);
      ...
      if (!isProcessingRef.current && !hasStartedPaymentRef.current && paymentState !== 'cancelled' && paymentState !== 'error' && !hasBeenCancelledRef.current) {
        console.log('iOS User is authenticated, starting payment process');
        hasStartedPaymentRef.current = true;
        await createCheckoutSessioniOS();
      } else {
        console.log('iOS Payment already in progress, cancelled, or error state - skipping');
      }
    };
  ...
  }, [params, createCheckoutSessioniOS, paymentState]);
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState: string) => {
      ...
      if (nextAppState === 'active' && paymentState === 'processing' && !isReturningFromStripe && !hasBeenCancelledRef.current) {
        console.log('iOS App returned to active, checking WebBrowser status');
        ...
        setTimeout(async () => {
          try {
            ...
            await WebBrowser.dismissBrowser();
            console.log('iOS WebBrowser was still open, now dismissed');
          } catch (error) {
            // WebBrowser is already closed, treat as cancellation
            console.log('iOS WebBrowser already closed, treating as cancellation');
            setPaymentState('cancelled');
            setIsProcessing(false);
            isProcessingRef.current = false;
            hasStartedPaymentRef.current = false;
  ...
  // Listen for deep links (when user returns from Stripe) - iOS specific
  useEffect(() => {
    const handleDeepLink = async (url: string) => {
      console.log('iOS Deep link received:', url);
      setIsReturningFromStripe(true);
      
      // iOS-specific: Dismiss the WebBrowser if it's still open
      try {
        await WebBrowser.dismissBrowser();
        console.log('iOS WebBrowser dismissed');
      } catch (error) {
        console.log('Error dismissing WebBrowser (may already be closed):', error);
      }
      ...
      if (url.includes('payment-success')) {
        ...
        if (sessionId) {
          ...
          console.log('iOS Payment successful, navigating to payment success screen...');
        ...
        console.log('iOS Payment cancelled via deep link');
        await handlePaymentCancellation();
      }
      // iOS doesn't need the fallback cancellation handling like Android
    };
              ...
              <TouchableOpacity 
                ...
                onPress={() => {
                  ...
                  if (timeSinceLastRetry < 2000) {
                    console.log('iOS Payment: Retry too soon, please wait');
                    return;
                  }
