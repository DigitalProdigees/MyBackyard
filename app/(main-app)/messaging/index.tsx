        ...
        if (userId) {
          ...
          const unsubscribe = onValue(conversationsRef, async (snapshot) => {
            if (snapshot.exists()) {
              ...
              console.log('Messaging: Real-time conversations update:', conversationsData);
              ...
              for (const [conversationId, conversationData] of Object.entries(conversationsData)) {
                ...
                if (conv && conv.contactId && conv.contactName) {
                  ...
                  console.log(`Messaging: Conversation ${conversationId} unreadCount:`, conv.unreadCount, '->', conversation.unreadCount);
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      if (returningFromChatRef.current) {
        console.log('Messaging: Returning from chat screen, resetting unread counts');
        ...
        const resetUnreadCounts = async () => {
          try {
            ...
            if (currentUserId) {
              ...
              console.log('Messaging: Current conversations before reset:', currentConversations.map(c => ({ id: c.id, name: c.contact.name, unreadCount: c.unreadCount })));
              
              for (const conversation of currentConversations) {
                if (conversation.unreadCount > 0) {
                  console.log(`Messaging: Resetting unread count for conversation ${conversation.id} (${conversation.contact.name}) from ${conversation.unreadCount} to 0`);
                  await chatService.markConversationAsRead(conversation.id, currentUserId);
                  console.log(`Messaging: Successfully reset unread count for conversation ${conversation.id}`);
                }
              }
              ...
              for (const conversation of currentConversations) {
                await chatService.markConversationAsRead(conversation.id, currentUserId);
                console.log(`Messaging: Force reset unread count for conversation ${conversation.id}`);
              }
              
              // Load conversations after resetting unread counts
              console.log('Messaging: Loading conversations after reset...');
  ...
  const handleConversationPress = (conversation: ChatConversation) => {
    ...
    router.push({
      ...
      params: {
        ...
        ownerId: conversation.contact.ownerId || conversation.contact.id,
        ...
        <View style={styles.conversationsContainer}>
          {isLoading ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateSubtext}>Start chatting with property owners from the backyard details page</Text>
            </View>
          ) : (
            conversations.map((conversation) => {
              ...
              // Check if last message is from current user (rental in main-app)
              const isLastMessageFromCurrentUser = conversation.lastMessage && 
                conversation.lastMessage.senderType === 'rental';
              ...
              const profileImage = profileImages[conversation.contact.id];
              console.log(`Messaging: Conversation ${conversation.contact.name} unread count:`, conversation.unreadCount);
              console.log(`Messaging: Passing unreadCount to ConversationItem:`, conversation.unreadCount || 0);
              return (
                <ConversationItem
                  key={conversation.id}
                  contact={{
